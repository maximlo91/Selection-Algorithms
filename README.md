...
sends an array into selection, with a randomly selected pivot being sent into the partition function, if selected k is the pivot it returns it. the function performes this recursively untill it finds the k element.\
sends an array into heap function, the function then creates a min heap with floyd's algorithem then the function removes k minimum numbers untill it reaches the k'th smallest.
sends an array into BST function, a BST is being created from the array, and then the k'ths smallest element is being found with a function that traverses the BST by the amount of left sub-tree nodes in each node untill it finds the k smallest node.
...
How to use test files: Test files as .txt documents are located in /tests after building the solution and/or\
compiling the program copy the contents of the test file and paste inside console. the program will produce the output.
